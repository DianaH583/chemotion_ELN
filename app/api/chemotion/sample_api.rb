    ...
    resource :samples do
          ...
          if extname.match(/\.(sdf?|mol)/i)
            sdf_import = Import::ImportSdf.new(file_path: params[:file][:tempfile].path,
                                               ...
                                               mapped_keys: {
                                                 ...
                                                 external_label: { field: 'external_label',
                                                                   displayName: 'External label' },
                                                 purity: { field: 'purity', displayName: 'Purity' },
                                                 molecule_name: { field: 'molecule_name',
                                                 ...
                                                 real_amount_unit: { field: 'real_amount_unit',
                                                                     displayName: 'Real Amount Unit' },
                                                 target_amount: { field: 'target_amount',
                                                                  displayName: 'Target Amount' },
                                                 target_amount_unit: { field: 'target_amount_unit',
                                                                       displayName: 'Target Amount Unit' },
      ...
      route_param :id do
        ...
        put do
          ...
          #   segment = Segment.find_by(element_type: Sample.name, element_id: @sample.id,
          #                             segment_klass_id: seg["segment_klass_id"])
          #   if segment.present?
          #     segment.update!(properties: seg["properties"])
          #   else
          #     Segment.create!(segment_klass_id: seg["segment_klass_id"], element_type: Sample.name,
          #                     element_id: @sample.id, properties: seg["properties"], created_by: current_user.id)
          #   end
          # end
